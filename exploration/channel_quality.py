...
HOW IT DECIDES (PREREG 8.1, amended by 8.2)
  ...
  PREREG 8.2 ruled the correlation criterion inapplicable to this montage. The
  first run flagged 419 of 2299 channel-subjects (18.2%) as bad-by-correlation
  while nan/flat/dropout fired 0 times, which is self-contradictory; flag counts
  track electrode eccentricity (Spearman rho = 0.58, p = 0.009); and the only
  outer-ring channels spared were Fp1/Fp2, the pair sharing an ocular common
  mode. The criterion therefore measures source sharing between electrodes, a
  property of spatial sampling density at 19 channels (nearest neighbour approx
  59 mm) rather than of channel quality. It is still computed and reported but
  no longer defines a sample.

    tier1    = nan | flat | dropout | hf_noise
    tier2    = tier1 | deviation                       <- PRIMARY sample
    prereg81 = tier2 | correlation                     <- invalidated, kept for
                                                          the record
  A subject holding >= 1 bad channel under a rung is excluded from that rung.
  Every rung is always reported together.
...
# PREREG 8.2: correlation no longer defines a sample (see module docstring).
TIER1 = ['nan', 'flat', 'dropout', 'hf_noise']
TIER2 = TIER1 + ['deviation']
PREREG81 = TIER2 + ['correlation']          # the rule as originally declared
LADDER = [('full', []), ('tier1', TIER1), ('tier2', TIER2),
          ('prereg81', PREREG81)]
...
def main(limit=None):
    ...
    # -- the three samples
    print('--- samples: every rung is always reported ' + '-' * 34)
    keeps = {}
    for name, crit in LADDER:
        keeps[name], _ = sample_report(rows, crit, name)
    print('  primary  = tier2')
    print('  prereg81 = the rule as first declared; invalidated by PREREG 8.2')
    print()
    print('  reference rung (deviation alone, no hf_noise):')
    sample_report(rows, ['nan', 'flat', 'dropout', 'deviation'], 'devonly')
    print()
    ...
    if roster is None:
        ...
        print('--- intersection with the ocular-IC roster (%d subjects) %s'
              % (len(roster), '-' * 20))
        for name, _crit in LADDER:
            inter = [r for r in keeps[name] if r['name'] in roster]
    ...
    with open(_OUT_DIR / 'samples.csv', 'w', newline='') as fh:
        ...
        for r in rows:
            ...
            w.writerow([r['name'], r['label'], '%.3f' % r['sec'],
                       ...
                       [int(not union(r['flags'], crit)) for _, crit in LADDER] +
